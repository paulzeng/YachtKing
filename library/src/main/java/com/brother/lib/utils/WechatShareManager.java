...
public class WechatShareManager {
    ...
    private void sharePicture(ShareContent shareContent, int shareType) {
        Bitmap bitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.icon_wechat_gzh);
        //Bitmap bitmap = BitmapUtils.decodeUriAsBitmap(Uri.parse(shareContent.getPictureResource()));
        ...
        bitmap.recycle();
        msg.thumbData = ShareBitmapUtils.bmpToByteArray(thumbBitmap, true);  //设置缩略图
    ...
    private void shareWebPage(final ShareContent shareContent, final int shareType) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    byte[] bytes = loadRawDataFromURL(shareContent.getPictureResource());
                    if(bytes == null) {
...
//		BitmapFactory.decodeStream(new URL(video.videoUrl).openStream());
        ...
        thumb.recycle();
        msg.thumbData = ShareBitmapUtils.bmpToByteArray(thumbBitmap, true);
