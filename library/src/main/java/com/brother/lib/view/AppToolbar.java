...
import com.brother.lib.view.dialog.SharePopupWindow;
...
public class AppToolbar extends Toolbar {
    ...
    private void init(Context context) {
        ...
        mTvRight =  findViewById(R.id.tv_toolbar_right_r);
    ...
    public void setRLDrawable(int resId) {
        if(resId==0){
            mIvRLBtn.setVisibility(GONE);
        }else{
            mIvRLBtn.setVisibility(VISIBLE);
    ...
    public void setRTClickListener(OnClickListener listener) {
    ...
    public void initShareListener(final View parent,final String title,final String content,final String image_url,final String target_url) {
        mIvRRBtn.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mShareWindow == null) {
                    View contentView = LayoutInflater.from(mContext)
                            .inflate(R.layout.layout_popup_share, null);
                    mShareWindow = new SharePopupWindow(contentView,title,content,image_url,target_url);
                }
                if (mShareWindow.isShowing()) {
                    mShareWindow.dismiss();
                } else {
    ...
    public void initShareListener(final View parent,final String type) {
        mIvRRBtn.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mShareWindow == null) {
                    View contentView = LayoutInflater.from(mContext)
                            .inflate(R.layout.layout_popup_share, null);
                    mShareWindow = new SharePopupWindow(contentView,type);
                }
                if (mShareWindow.isShowing()) {
                    mShareWindow.dismiss();
                } else {
